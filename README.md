...
`<ProjectName>References` interface holding each declared image's full docker identifier
...
For a project named `fixture` this generates `com.example.FixtureReferences`:

```java
package com.example;

// Generated. Do not edit.
public interface FixtureReferences {
    public static final String APP = "example/app:1.0";
    public static final String WEB_SERVER = "example/web:2.0";
}
```

The interface is **shared**: `<ProjectName>References` is generated by a single
`generateReferences` task (appending the capitalized source-set name for non-`main` sets —
image references are always `main`, so there is no suffix). When the helm and/or
generic-artifacts plugins are also applied with references enabled, their constants are
**merged into the same interface** rather than colliding, so a project can expose image
tags, chart resource paths, and arbitrary strings side by side in one `FixtureReferences`.
...
(`<ProjectName>ReferencesTest`, etc.). This is the same shared `<ProjectName>References`
interface the container and generic-artifacts plugins write into, so their constants merge
when several plugins are applied. When the `eclipse` plugin is also applied, `eclipseClasspath`
carries both the generated source folder and the chart resource folders.
...
For a project named `fixture` this generates `com.example.FixtureReferences`, and the
...
// e.g. getClass().getClassLoader().getResourceAsStream(FixtureReferences.API)
```
...
source set land in a suffixed `<ProjectName>ReferencesTest`, etc.).
...
image's tag (an arbitrary string, not a resource path) into the `<ProjectName>References` interface.
All three plugins (container, helm, generic-artifacts) contribute to that **one** shared interface
per source set through a single `generateReferences` task, so when several are applied their
constants are merged rather than emitting colliding files.
