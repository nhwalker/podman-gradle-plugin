...
The interface name (`<ProjectName>References`, shared across the container, helm, and
generic-artifacts plugins) appends the capitalized source-set name for any non-`main`
source set — image references are always `main`, so there is no suffix.
...
(`<ProjectName>ReferencesTest`, etc.). When the `eclipse` plugin is also applied,
`eclipseClasspath` carries both the generated source folder and the chart resource folders.
...
both now flow through the same `GenerateReferencesTask`.
